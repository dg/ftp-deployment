...
- `RetryServer` wraps any server implementation with automatic retry logic; also reconnects after `SkipException`/`TerminatedException` since interrupted transfers leave the FTP connection in a broken state
- `FtpServer` has built-in FTPS reconnect: SSL data connections are prone to intermittent failures, so `readFile`/`writeFile` automatically reconnect and retry on error (max 3 reconnects for uploads)
- `FtpServer::connect()` abandons old connection resource instead of calling `ftp_close()` which can block indefinitely after interrupted transfers (QUIT hangs while server waits for data)
...
- Handles Ctrl+C with interactive prompt: [s]kip / [r]esume / [c]ancel
- Flag-based polling: signal handler sets flag, `check()` called at strategic points
- Mid-upload interruption via progress callback in `uploadPaths()`
- Fragmented sleep in `RetryServer` (200ms intervals) for responsive Ctrl+C
- `SkipException` propagates skip decision, `TerminatedException` propagates cancel
- Skipped uploads/renames/deletes trigger deployment file regeneration via `revertSkippedPaths()` to keep server state consistent
- Non-interactive mode (CI/CD): terminates immediately on Ctrl+C
- Skipped operations tracked and reported at end of deployment
...
- PHP 8.2+ required, all files must have `declare(strict_types=1)`
...
- Interrupt handling tests use `TestInterruptHandler` subclass that overrides `check()` to throw at predetermined operations, avoiding signal/STDIN dependencies
